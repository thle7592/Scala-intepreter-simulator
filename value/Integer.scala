...
case class Integer(val value: Int) extends Literal with Ordered[Integer] with Equals {
  ...
  def -(other: Integer) = Integer(this.value - other.value)
  def *(other: Integer) = Integer(this.value * other.value)
  def /(other: Integer) = if(other.value == 0) throw new Exception("Division by zero") else Integer(this.value/other.value)
  def unary_- = Integer(-1 * value) // unary negation
  ...
  //override def execute(env: context.Environment) = new Integer(value)
}
